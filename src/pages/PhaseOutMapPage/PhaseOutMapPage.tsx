 ...
 * PhaseOutMapPage.tsx
 *
 * - Shows map with oil/gas fields
 * - Allows phase-out of fields and updates state
 * - Uses useReducer to handle complex state
 *
 * NOTE: The person with the best knowledge of the entire data structure should handle the integration? */
  ...
  try {
    ...
    if (saved && saved.length <= 100000) {
      ...
      if (
        ...
        const validStatutes = ["active", "closed", "transitioning"] as const;

        const parsedFields: Field[] = parsed.gameFields
          .filter((f: any) => validStatutes.includes(f.status))
          .map((f: any) => ({
            ...f,
            status: f.status as "active" | "closed" | "transitioning",
          }));
        return {
          ...parsed,
          gameFields: parsedFields,
          ...
          totalProduction: parsed.total || 0,
        };
      }
    }
  } catch (e) {
...
const getColorForIntensity = (
  ...
  // tailwindcss colors, if we decide to use tailwindcss
...
const gameReducer = (state: GameState, action: GameAction): GameState => {
  switch (action.type) {
    ...
    case "PHASE_OUT_FIELD": {
      ...
      const newGameFields: Field[] = state.gameFields.map((f) => {
        if (f.name === fieldName) {
          return {
            ...f,
            status: "closed" as "closed", // 👈 Eksplisitt type
            production: 0,
          };
        }
        return f;
